		...
		{assign var='_bgColor' value=$rowData.customRowColor} 
		 ...
		 <tr height='20' class='{$_rowColor}S1' bgcolor='{$_bgColor}'>
			...
			{foreach from=$displayColumns key=col item=params}
			    {strip}
				<td scope='row' align='{$params.align|default:'left'}' valign="top" {if ($params.type == 'teamset')} class="nowrap"{/if} bgcolor='{$_bgColor}'>
					 ...
					 {elseif $params.type == 'multienum'}
						{if !empty($rowData.$col)} 
							...
							{foreach from=$vals item=item}
								{counter name="oCount"}
								{sugar_translate label=$params.options select=$item|replace:"^":"" }{if $oCount !=  count($vals)},{/if} 		
							{/foreach}	
						{/if} 
					{else}	
